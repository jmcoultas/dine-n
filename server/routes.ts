    ...
    try {
      ...
      if (recipeId < 0) {
        ...
        const transformedRecipe = {
          ...
          instructions: `{${Array.isArray(recipeData.instructions) ? recipeData.instructions.map(i => `"${i.replace(/"/g, '\\"')}"`) : []}}`,
          tags: `{${Array.isArray(recipeData.tags) ? recipeData.tags.map(t => `"${t.replace(/"/g, '\\"')}"`) : []}}`,
